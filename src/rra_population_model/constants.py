...
# it emitted `*_residential_density` for every version though nothing produces
# it (a KeyError on `pixel_ghsl_r2023a_residential_density` at training time),
# while having no way to name a provider that ships one measure and is not a
# registered BuiltVersion.
#
# `microsoft_v8_obm_...` is that case: Microsoft v8's volume re-split by OBM's
# residential fraction, written by `process_msft_obm` as its only real measure.
# Registering it as a BuiltVersion to reach a cross-product would oblige it to
# publish density and volume aliases of rasters already on disk under their
# canonical microsoft_v8 names - see MSFT_V8_OBM_LINKED_MEASURES. Naming the
# raster directly costs nothing and keeps that tuple empty.
#
# OBM appears here only in combination with Microsoft v8, never on its own. OBM
# covers 2,902 of the 4,797 modelling-frame blocks (60.5%), so a standalone
# `obm_20250404_*` denominator would be nodata across 39.5% of the frame and the
# model would train on a target defined for only part of the world.
# `microsoft_v8_obm_...` has no such hole: it is v8's volume - full coverage -
# re-split by OBM's residential fraction where OBM exists, so it stays defined
# wherever v8 is. That combination is worth training on precisely because it
# differs from plain v8: on 8 of 12 sampled blocks it moves total residential
# volume by up to 19%.
#
# `obm_20250404` therefore stays in BUILT_VERSIONS - the features step must
# still build its rasters, which is where `microsoft_v8_obm_...` comes from -
# but is deliberately not a denominator.
#
# Every entry must exist as a feature raster; check against
# `PopulationModelData.list_features` before adding one.
DENOMINATORS = [
    *(
        f"{provider}_{measure}"
