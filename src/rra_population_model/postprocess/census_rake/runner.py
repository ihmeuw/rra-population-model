...
def census_rake(
    ...
    census_tasks = task_admins.reset_index().loc[:, ["iso3", "census_time_point", "task_parent_id"]].drop_duplicates()
    ...
    census_tasks = parallel.run_parallel(
        _check_complete,
        possible_census_tasks,
        num_cores=10,
    )
    census_tasks = [i for i in census_tasks if len(i) > 0]

    print(f"Building raking factors for {len(census_tasks):,} census time-admins (out of a possible {len(possible_census_tasks):,}).")
    jobmon.run_parallel(
